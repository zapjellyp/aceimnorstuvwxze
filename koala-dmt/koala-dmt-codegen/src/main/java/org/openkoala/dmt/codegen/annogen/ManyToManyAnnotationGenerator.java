...
public class ManyToManyAnnotationGenerator implements FieldAnnotationGenerator {
	...
	public List<AnnotationExpr> generateAnnotations(PropertyInfo propertyInfo) {
		...
		if (StringUtils.isBlank(propertyInfo.getMappedBy())) {
			results.add(new MarkerAnnotationExpr(new NameExpr("ManyToMany")));
			...
			results.add(new NormalAnnotationExpr(new NameExpr("ManyToMany"), pairs));
		}
		return results;
	}
